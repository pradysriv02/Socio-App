...
const ChatBox = ({
  ...
  const getMessages = async () => {
    const response = await fetch(
      `http://localhost:3001/messages/${chatData?._id}`,
      {
        method: "GET",
        headers: { Authorization: `Bearer ${token}` },
      }
  ...
  return (
    <Box>
      <Box borderRadius="1rem" display="grid" gridTemplateRows="14vh 60vh 13vh">
        {chatData ? (
          <Box padding="1rem 1rem 0 1rem" display="flex" flexDirection="column">
            <Box
              display="flex"
              gap="2rem"
              onClick={() => navigate(`/profile/${otherUser?._id}`)}>
              <UserImage image={otherUser ? otherUser.picturePath : null} />
              <Typography variant="h4" mt="1.2rem">
          ...
          <Box
            ...
            width="50%"
            margin="auto">
            <Typography variant="h2" fontFamily="Arial,sans-serif">
              Click on a chat and......TALK....
            </Typography>
          </Box>
        )}
        <Box
